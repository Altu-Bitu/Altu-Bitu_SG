...
int calcScore() {
    ...
    for (int i = 0; i < n; i++) { //i: 이닝
        ...
        while (out < 3) {
            ...
            if (idx == 10) //9번째 선수까지 다 했으면
                idx = 1; //맨 처음 선수로 다시 돌아가기
            switch (state) {
                case 0://아웃
                    out++;
                    break;
                case 1: //안타, 1루씩 진루
                    score += baseman[3]; //3루에 있는 선수있다면 홈 도착(1점 추가)
                    for (int i = 3; i > 0; i--)
                        baseman[i] = baseman[i - 1]; //2루에 있는 선수는 3루로, 1루에 있는 선수는 2루로
                ...
                case 3: //3루타, 3루씩 진루
                    for (int i = 3; i > 0; i--) { //3루, 2루, 1루에 선수 있다면 홈 도착
                        score += baseman[i];//3명의 선수가 모두 홈에 들어오니까 다 1점씩 더해주고
                        baseman[i] = 0;//베이스 다 비워주기
                    }
